...
def factures_fournisseurs_result():
    ...
    print("Fact columns:", df_fact.columns.tolist())
    print("Fourn cols:", df_fourn.columns.tolist())


    # Supprimer les espaces des en-têtes
    df_fact.columns = df_fact.columns.str.strip()
    df_fourn.columns = df_fourn.columns.str.strip()

    # Fusionner sur la clé exacte
    df = pd.merge(df_fact, df_fourn, on='Nom du fournisseur', how='left')


    # 2) Appliquer les filtres depuis GET
    args = request.args
    if args.get('code_fournisseur'):
        df = df[df['Code fournisseur'] == args['code_fournisseur']]
    if args.get('nom_fournisseur'):
        df = df[df['Nom du fournisseur'].str.contains(args['nom_fournisseur'], case=False)]
    if args.get('po_number'):
        df = df[df['N° de PO'] == args['po_number']]
    # Filtrer sur le statut de paiement avec la colonne réelle "Paiement"
    if args.get('statut_paye') and args['statut_paye'] != 'ensemble':
        wanted = 'payée' if args['statut_paye']=='payée' else 'non-payée'
        df = df[df['Paiement'] == wanted]

    if args.get('date_facture_de'):
        df = df[df['Date de facture'] >= args['date_facture_de']]
    if args.get('date_facture_a'):
        df = df[df['Date de facture'] <= args['date_facture_a']]
    if args.get('date_echeance_de'):
        df = df[df["Date d'échéance"] >= args['date_echeance_de']]

    if args.get('date_echeance_a'):
        df = df[df["Date d'échéance"] >= args['date_echeance_a']]
