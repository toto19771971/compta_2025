...
db_conn = sqlite3.connect('grand_livre.db', check_same_thread=False)

# ────────────────────────────────────────────────────────
# Route UNIFIÉE POUR CRÉER LA LIGNE + PUBLIEZ DANS LE GL
# ────────────────────────────────────────────────────────
@app.route('/creer_et_publier', methods=['POST'])
def creer_et_publier():
    # 1) Récupérer TOUTES les données du formulaire
    form_data = request.form.to_dict()

    # ─── AJOUT ─── Récupérer module & orig_id (inputs cachés)
    module  = form_data.get('module')    # "fournisseurs"
    orig_id = form_data.get('orig_id')   # identifiant unique Excel

    # 2) INSERT dans grand_livre.db
    cols         = ', '.join(form_data.keys())
    placeholders = ', '.join('?' for _ in form_data)
    sql          = f"INSERT INTO grand_livre ({cols}) VALUES ({placeholders})"
    cur          = db_conn.cursor()
    cur.execute(sql, tuple(form_data.values()))
    db_conn.commit()

    # 3) Récupérer new_id auto‑généré
    num_ecriture = cur.lastrowid

    # 4) Mettre à jour l’Excel du module
    excel_path = f"bd_{module}.xlsx"       # Chemin VOTRE dossier
    df         = pd.read_excel(excel_path, dtype=str, keep_default_na=False)

    # 4a) Filtrer la ligne via orig_id
    mask = df['orig_id'].astype(str) == str(orig_id)

    # 4b) Inscrire le numéro d’écriture
    df.loc[mask, "Numéro d'écriture"] = num_ecriture

    # 4c) Sauvegarder
    df.to_excel(excel_path, index=False)

    # 5) Retour JSON au front
    return jsonify({"num_ecriture": num_ecriture}), 200


# ────────────────────────────────────────────────────
# Chargement des DataFrames & création des index (au start)
# ────────────────────────────────────────────────────
df_four  = pd.read_excel('bd_factures_fournisseurs.xlsx', dtype=str, keep_default_na=False)
orig_col = next(col for col in df_four.columns if 'écriture' in col.lower())
df_four = df_four.rename(columns={orig_col: 'num_ecriture'})
...
idx_four = set(df_four['num_ecriture'])
#idx_cli  = set(df_cli ["Numéro d'écriture"])
#idx_sal  = set(df_sal ["Numéro d'écriture"])


# ───────────────────────────────────────────────────────────────
# Route UNIQUE pour ÉDITION (clic sur num_ecriture dans le GL)
# ───────────────────────────────────────────────────────────────
...
def editer_ecriture(num_ecriture):
    print(f"[DEBUG] num_ecriture reçu = {num_ecriture}")
    if num_ecriture in idx_four:
        df, tpl = df_four, 'factures_fournisseurs_ecriture.html'
    else:
        row = pd.read_sql_query(
            """SELECT * FROM Sheet1 WHERE "numéro d\'écriture" = ?""",
            db_conn, params=(num_ecriture,)
        ).iloc[0]
        return render_template('templates_comptabilite/grand_livre_ecriture.html', data=row.to_dict())
    row = df.loc[df['num_ecriture'].astype(int) == num_ecriture].iloc[0]
    return render_template(tpl, data=row.to_dict())
...
def double_creation():
    ...
    conn = engine.raw_connection()
    cur  = conn.cursor()
    # 🟢 Boucle existante pour insérer 3 écritures :
    ...
    # 🟢 (Optionnel) ajouter colonne si manquante
    try:
        cur.execute('ALTER TABLE Sheet1 ADD COLUMN "Intitulé du compte" TEXT')
    except:
        pass
    
    # 🟩 Récupérer le dernier numéro existant et incrémenter manuellement
    for i in range(len(comptes_ht)):
        ...
        cur.execute("""INSERT INTO Sheet1
            ...
            no_facture, form_data.get('Fournisseur',''),
            0, montant
        ))
        # b) écriture HT
    ...
    conn.commit()
    # 🟢 Récupérer le numéro d’écriture généré
    num_ecriture = cur.lastrowid
    cur.close()
    ...
    fp = os.path.join(app.root_path, 'bd_factures_fournisseurs.xlsx')
    df = pd.read_excel(fp, dtype=str, keep_default_na=False)
    # 🔵 Construire une nouvelle ligne = tous les champs + num_ecriture
    new_row = {
      'Fournisseur'           : form_data['Fournisseur'],
      ...
      'Date échéance'         : form_data['Date échéance'],
      ...
      'Paiement'              : '',
      'Numéro d\'écriture'    : str(num_ecriture)   # 🔵 nouvelle colonne
    }
    df.loc[len(df)] = new_row
    df.to_excel(fp, index=False)
    
    # 🔵 # 🔵 4) Renvoi JSON avec le numéro pour le front
    return jsonify({"num_ecriture": num_ecriture}), 200
